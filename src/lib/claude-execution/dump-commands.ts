...
export type DumpExistingCommitment = { id: string; title: string; kind: CommitmentKind };
...
export function buildDayDumpPrompt(input: {
  ...
  return [
    ...
    "Set nothing_found true only when items is empty and no actionable commitment or idea appears in BRAIN_DUMP.",
...
export function validateDayDump(
  ...
  exactKeys(root, ["items", "skipped_duplicates", "nothing_found"], "root");
  ...
  const items = root.items.map((entry, index): DumpExtractionItem => {
    ...
    const sourceQuote = stringValue(item.source_quote, `item_${index}_source_quote`, 8000)!;
    if (!normalizedDump.includes(normalizeWhitespace(sourceQuote))) {
      throw new DayDumpInvalid(`item_${index}_source_quote_not_verbatim`);
    }
  ...
  if (root.nothing_found && (items.length > 0 || skippedDuplicates.length > 0)) {
