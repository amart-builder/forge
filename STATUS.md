...
- **Morning Brief engine v1 is being built** from the post-Sol-review spec (session scratchpad `morning-brief-spec.md`; fold into repo docs when it lands). Shape: Forge-owned context collector (GOALS.md + sprint memo + task snapshot + settlement summary required; email card + jarvis-memory optional adapters with freshness/coverage manifest), dedicated brief worker lane (must not starve behind execution runs), toolless Opus session returning strict-schema brief (lens_narrative, 3 existing_task_candidates with why_today/owner/what_claude_can_start, suggested_additions inbox, watch_items, sales_actions with draft_kind + approval_required), immutable artifacts keyed by composite input hash, arrival rehydrates candidates from fresh task evidence and never hot-swaps an interacted arrival, fail-open to deterministic everywhere, brief_json loopback-only. Triggers: post-settlement-reconciliation, ~7:30 schedule, on-demand backfill.
